...
(function() {
  ...
  app.Record = Backbone.Model.extend({
    ...
    url: function() {
      return '/admin/subscriptions/'+ (this.isNew() ? '' : this.id +'/');
    }
  });
  ...
  app.HeaderView = Backbone.View.extend({
    ...
    addNew: function() {
      ...
      this.$el.find('input').each(function(index, el) {
        if(el.value === '' && valid) {
          alert('Please complete ' + el.placeholder + '.');
          valid = false;
        }
      });
      ...
      if (valid) {
        this.model.save({
          header: {
            name: this.$el.find('[name="header.name"]').val(),
            description: this.$el.find('[name="header.description"]').val(),
            language: this.$el.find('[name="header.language"]').val(),
            genre: this.$el.find('[name="header.genre"]').val().split(','),
          },
        },{
